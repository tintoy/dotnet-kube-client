        ...
        : Controller
    {
        ...
        public async Task<IActionResult> Exec(string kubeNamespace, string podName)
        {
            ...
            WebSocket webSocket = await HttpContext.WebSockets.AcceptWebSocketAsync(
                subProtocol: K8sWebSocketSubprotocols.ChannelV1
            );

            if (WebSocketTestAdapter.OnPodExec != null)
                await WebSocketTestAdapter.OnPodExec(webSocket);
            else
                await webSocket.CloseAsync(WebSocketCloseStatus.EndpointUnavailable, "WebSocketTestData.OnAcceptConnection is not configured.", CancellationToken.None);
